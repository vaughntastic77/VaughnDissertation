...
Here we test the usage of references. The book\cite{aiaa09} 
is referred in this way. Actually, the option is available for you to change the default way how reference appears. The default and most commonly used option \cite{aiaa10} is displayed here \cite{saric}.Both of these options are acceptable. 

Unrelated citations are referred here for the test of reference section only\cite{aiaa12}. If you
find that the reference \cite{aiaa19} has more items than you need \cite{pope}, question marks will show up in place of a reference handle, like these \cite{tichenor,mai}.
